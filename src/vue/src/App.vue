...
import ArgConfigPage from './components/ArgConfigPage.vue';
import { ref } from 'vue';
import { triggerFakeEvent } from './components/store';

const loading = ref(false);
const content = ref('jsonTable');
...
function switchPage(){
    content.value = content.value == 'jsonTable' ? 'config' : 'jsonTable';    
}
...
<template>
    ...
    <ArgConfigPage v-if="!loading && content=='config'" @switchPage="switchPage()" /> 
    <ArgJsonTable v-if="!loading && content=='jsonTable'" :key="contentKey" @switchPage="switchPage()" @refresh="refreshPage()" />
</template>
