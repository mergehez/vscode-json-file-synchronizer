...
import {currentWorkspacePath, postMessageToVsCode, subscribeToVsCodeResponse} from "../helpers";
    ...
    <div class="py-3 px-1 flex gap-3" style="width: min(800px, 80dvw)">
        <form class="w-7/12" @submit.prevent="requestFilePaths()">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="input-fileExts" :required="true" v-model="currConfig.fileExts" placeholder="e.g. json">
            </div>
            <div class="flex mb-3">
                <label for="input-recursive" class="leading-tight px-1 text-sm" style="width: 140px;">Search recursively</label>
                <div class="flex-1">
        ...
        <div class="w-5/12 border-l border-gray-400 dark:border-gray-500 pl-3">
            ...
            <div v-else class="h-full flex items-center justify-center">
                <span class="alert alert-danger text-xs mt-2 max-w-full break-all"> {{ searchResultMessage }}</span>
            </div>
        </div>
    </div>
</template>