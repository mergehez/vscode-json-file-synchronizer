...
function selectConfig(c: Config) {
    ...
    loading.value = true;
    return postMessageToVsCode({request: 'getTranslations', data: c})
}

async function deleteConfig(c: Config) {
    postMessageToVsCode({request: 'deleteConfig', data: c})
    allConfigs.value = allConfigs.value.filter((t: Config) => t.key != c.key);
    if (selectedConfig.value?.key === c.key) {
        selectedConfig.value = null;
        content.value = 'selector';
    }
}

state.fake = !window.location.href.startsWith('vscode-webview:') && !window.location.href.includes('/jbcefbrowser/');
