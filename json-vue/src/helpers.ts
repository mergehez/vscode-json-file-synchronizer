...
const vscode = 'vscode' in window ? window.vscode : Function(` return typeof acquireVsCodeApi == 'function' ? acquireVsCodeApi() : undefined; `)();
...
export function postMessageToVsCode(message: CommandFromVue, loaderTarget?: string){
    console.log("postMessageToVsCode", message, loaderTarget, vscode)
    if(!loaderTarget) {
